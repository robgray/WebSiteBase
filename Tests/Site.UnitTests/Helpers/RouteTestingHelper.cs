...
using Mvc.Tests.Installers.Plumbing;
...
namespace Mvc.Tests.Helpers
{
    ...
    public static class RouteTestingExtensions
    {
        ...
        private static HttpContextBase FakeHttpContext(string url, HttpVerbs? httpMethod, HttpVerbs? formMethod)
        {
            ...
            var context = MvcMockHelpers.FakeHttpContext(url);
            context.Request.AppRelativeCurrentExecutionFilePath.Returns(url);
            context.Request.PathInfo.Returns(string.Empty);
            context.Request.Form.Returns(form);
            context.Request.HttpMethod.Returns(httpMethod.Value.ToString().ToUpper());

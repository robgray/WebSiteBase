...
namespace Mvc.Tests.Helpers
{
    /// TAKEN FROM : http://mvccontrib.codeplex.com/SourceControl/changeset/view/93c3fbd0f8cc#src/MvcContrib.TestHelper/MvcContrib.TestHelper/ActionResultHelper.cs
    ...
    public static class ActionResultHelper
    {
        ...
        public static TModel WithModel<TModel>(this ViewResult actionResult)
        {
            return AssertHasModelType<TModel>(actionResult);
        }

        private static TModel AssertHasModelType<TModel>(ViewResultBase actionResult)
        {
            var actualModel = actionResult.Model;
            var expectedType = typeof(TModel);

            if (actualModel == null)
            {
                throw new ActionResultAssertionException(string.Format("Expected model of type '{0}', actual was NULL",
                                                                       expectedType.Name));
            }
            
            if (!typeof(TModel).IsAssignableFrom(actualModel.GetType()))
            {
                throw new ActionResultAssertionException(string.Format("Expected view data of type '{0}', actual was '{1}'",
                                                                       typeof(TModel).Name, actualModel.GetType().Name));
            }
